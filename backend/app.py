...
def createSkloniste():
    ...
    sql = f'''
    INSERT INTO skloniste(id, naziv, adresa, id_mjesta)
    VALUES({request_data["id"]}, '{request_data["naziv"]}',
...
def getSkloniste(id_sklonista):
    ...
    FROM ( SELECT
    s.id, s.naziv, s.adresa, m.naziv as naziv_mjesta
    FROM skloniste s
    JOIN mjesto m
    ON s.id_mjesta = m.id
    WHERE m.id = {id_sklonista})
    as r
    '''
    cursor.execute(sql)
...
@app.route('/skloniste/<int:id_sklonista>/zivotinja/<int:broj_cipa>')
def getZivotinja(id_sklonista, broj_cipa):
    sql = f'''
    SELECT row_to_json(r, true)
    FROM ( SELECT
...
@app.route('/skloniste/<int:id_sklonista>/zivotinja/<int:broj_cipa>', methods=['PUT'])
def updateZivotinja(id_sklonista, broj_cipa):
    request_data = request.get_json()
...
@app.route('/skloniste/<int:id_sklonista>/zivotinja/<int:broj_cipa>', methods=['DELETE'])
def deleteZivotinja(id_sklonista, broj_cipa):
    sql = f'''
